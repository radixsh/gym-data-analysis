...
def main():
    ...
    # all_tables = cur.execute("SELECT name FROM sqlite_master")
    # pprint(all_tables.fetchall())
    ...
    # res = cur.execute(f"SELECT * FROM {table} WHERE name == 'bench press'")
    ...
    # TODO: Choose an exercise, and find its data across all workouts
    # Graph that data 

    # table = 'workout_set_group_table'
    # bench_press = 
    # res = cur.execute(f"SELECT row FROM {table} WHERE row.exerciseId ==
    #                   {bench_press}")
    # pprint(res.fetchall())
